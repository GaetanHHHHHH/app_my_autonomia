...
export class ActualitesService {
    ...
    addActualite(formValue: { titre: string, titre2: string, texte: string, 
        id_users: number, vignette: string, lien?: string}): Observable<Actualite> {
    ...
    deleteActualite(actualiteId: number): Observable<Actualite> {
        return this.http.delete<Actualite>(`http://localhost:8080/api/actualites/${actualiteId}`)
    }
}